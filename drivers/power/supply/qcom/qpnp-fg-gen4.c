...
{
	struct fg_dev *fg = container_of(work, struct fg_dev, soc_work.work);
	int soc = 0, temp = 0;
	int rc;
	...
	schedule_delayed_work(&fg->soc_work, msecs_to_jiffies(SOC_WORK_MS));
}
...
{
	int rc;
	struct fg_dev *fg = container_of(work, struct fg_dev, soc_monitor_work.work);
	struct fg_gen4_chip *chip = container_of(fg, struct fg_gen4_chip, fg);
